...
def material_names(scenario: Dict[str, Any], material_ids: List[str]) -> List[str]:
    """材料 id -> 中文名。

    材料在场景配置里用稳定 id 存储，但**面向用户只能出现中文名**（见 docs/08 文案规范）。
    取不到条目时回退成 id 本身，保证不会因为配置缺项而崩。
    """
    catalog = scenario.get("materials") or {}
    return [catalog.get(mid, {}).get("name", mid) for mid in (material_ids or [])]
